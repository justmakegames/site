...
class qtczoneShipHelper
{
	 ...
	 */
	function _viewHandler($jinput, $plugview = 'default')
	{
	...
	function _qtcLoadViewData($plugLayout = 'default', $jinput)
	{
		...
		return $data;//$aretirnUrlPramStr = $qtczoneShipHelper->$viewhandler($jinput);
	}
...
//    $logs->addEntry(array('user' => $my->name.'('.$my->id.')','desc'=>json_encode($logdata['raw_data'])));
	...
	function loadViewDataForSetratesView($jinput)
	{
		$taxHelper = new taxHelper;
		$qtczoneShipHelper = new qtczoneShipHelper;
		$qtcshiphelper = new qtcshiphelper;
	 ...
	 * @return   Method name.
	 */
	function getShipTypeName($methType)
	{
		$methodName = "";

		switch($methType)
		{
			case 3: return  JText::_('PLG_QTC_DEFAULT_ZONESHIPPING_STORE_ITEM');
			break;

			case 1: return  JText::_('PLG_QTC_DEFAULT_ZONESHIPPING_STORE_QTY');
			break;

			case 2: return  JText::_('PLG_QTC_DEFAULT_ZONESHIPPING_STORE_WEIGHT');
			break;
		}
	}
	 ...
	 */
	function getAvailableShipMethods($store_id)
	{
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
	 ...
	 * @return   Array of shipping method detail.
	 */
	function getShipMethodDetail($shipMethId)
	{
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
		$query->select(" id AS methodId,name,store_id,taxprofileId,state,shipping_type,min_value,max_value")
	...
	function getApplicableShipMethRateDetail($vars, $shipMethDetail)
	{
		...
		if (!empty($shipMethDetail))
		{
			...
			elseif ($shipMethDetail['shipping_type'] == 2)
			{
				...
				// Check whether mehod
				if (((float)$shipMethDetail['min_value'] <= $newItemWt) && ((int)$shipMethDetail['max_value'] == -1 || $newItemWt <= (float)$shipMethDetail['max_value']))
				{
					$applicable = 1;
				}
	 ...
	 */
	function getPriceRelMethDetail($methodId, $currency)
	{
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
	 ...
	 * @return  shipping method rate Details.
	 */
	function getShipMethRateDetail($shipMethRateId, $currency)
	{
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
...
*/
	 ...
	 * @return  symbol.
	 */
	function getWeightUniteSymbol()
	{
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
		//$query->select('w.id,w.unit');
		$query->select('w.unit');
