...
var instanceOptions=instance.options=extend(!0,{},Class.defaults,Class.defaultOptions,options),element=instance.element=$(elem);if(Class.protoFactory&&(prototype=Class.protoFactory.apply(Class,[instance,instanceOptions,element]),extend(!0,instance,prototype)),(data(elem)||data(elem,{}))[_fullName]=instance,!element.data("directSelector")){var selector=$.uid("DS");element.addClass(selector).data("directSelector","."+selector)}for(var name in instanceOptions)if(name.match(/^\{.+\}$/)){var key=name.replace(/^\{|\}$/g,""),val=instanceOptions[name];
if(isString(val)){var selectorFuncExtension=instance[key];instance[key]=function(instance,selector,funcName){selector=/^(\.|\#)$/.test(selector)?selector+funcName:selector;var selectorFunc=function(filter){var elements=instance.element.find(selector);return $.isString(filter)&&(elements=elements.filter(filter)),$.isPlainObject(filter)&&$.each(filter,function(key,val){elements=elements.filterBy(key,val)}),elements};return selectorFunc.selector=selector,selectorFunc.css=function(){var cssRule=selectorFunc.cssRule;
