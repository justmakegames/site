...
},isFluidImage:function(a){return b.imageContainer.inside(a).hasClass(f)},isFixedImage:function(a){return!b.isFluidImage(a)},toFixedImage:function(a){var c=B.data(a),d=b.imageElement.inside(a),e=d.width(),f=d.height();b.setImageWidth(a,e);var g=c.ratio_lock;g||b.setImageHeight(a,f)},toFluidImage:function(a){var c=B.data(a),d=b.imageContainer.inside(a),e=b.imageElement.inside(a);if(B.isRootBlock(a))var f=d.width(),g=a.width(),h=Math.round(f/g*100)+"%";if(B.isNestedBlock(a))var h="100%";var i=e.height();b.setImageWidth(a,h);
var j=c.ratio_lock;j||b.setImageHeight(a,i)},getImageSizeField:function(c){var d=b["imageSizeFieldImage"+a.capitalize(a.camelize(c))]();return d},getImageSizeProp:function(a){var c=a.closest(b.numslider.selector),d=m(c.data("name"));return d},getImageSizeUnit:function(c){var d=b.getImageSizeField(c);return a.trim(b.numsliderCurrentUnit.under(d).text())},handleNumsliderWidget:function(a,c){var d=b.getImageSizeProp(a),e=b.getImageSizeUnit(d),c=Math.round(c)+e;b.resizingFromSlider=d,b.updateImageSize(D,d,c),b.resizingFromSlider=null
...
break;case c.ENTER:i.length>0&&(h.use(i),e.preventDefault());break;default:g.populateFromTextfield()}},"{item} mouseover":function(a){h.deactivate(),h.activate(a)},"{item} mouseout":function(){h.deactivate()},"{item} click":function(a){h.use(a)},"{textboxlist} addItem":function(b,c,d){a(h.get(d.title)).addClass("is-used")},"{textboxlist} removeItem":function(b,c,d){d.assigned?i.addItem(d.title):a(h.get(d.title)).removeClass("is-used")},"{textboxlist} listChange":a.debounce(function(){g.refresh();var a=b.tagger.getAddedItems().length;
b.count().text(a)},15),"{self} categoryselect":function(b,c,d){if(!(d.tags.length<=0)){var e=(d.tags||"").split(",");a.each(e,function(a,b){i.addLockedItem(b)})}},"{self} categorydeselect":function(c,d,e){if(!(e.tags.length<=0)){var f=b.composer.category.toArray(),g=(e.tags||"").split(","),h=a.pluck(f,"tags").join(",").split(","),j=a.without.apply(null,[g].concat(h));a.each(j,function(a,b){var c=i.get(b);c&&i.removeItem(c.id)})}},"{toggleButton} click":function(){g[b.suggestions().hasClass("is-showing")?"collapse":"expand"]()
},"{autofillButton} click":function(c){var d=b.composer.document.getText(),e=a(c).parent();a(e).addClass("is-loading"),EasyBlog.ajax("site/views/composer/suggestKeywords",{data:d}).done(function(b){a(e).removeClass("is-loading"),b&&a.each(b,function(a,b){i.addItem(b.title)})})},suggestions:{index:0,tags:[],expand:function(){g.tick++,b.suggestions().addClass("is-showing")},collapse:function(){g.tick++,b.suggestions().removeClass("is-showing")},refresh:function(){var a=b.item(":not(.is-used)");b.suggestions().toggleClass("is-empty",a.length<1),g.count()
},show:function(){g.tick++,g.refresh(),g.expand()},hide:function(){var a=++g.tick;g.collapse(),setTimeout(function(){a===g.tick&&g.reset()},500)},set:function(a){a.length<1?(g.hide(),g.tags=[]):(g.reset(),g.tags=a)},reset:function(){g.tags=[],g.i=0,b.itemgroup().empty()},pid:0,tick:0,currentQuery:null,query:a.memoize(function(a){return EasyBlog.ajax("site/views/tags/suggest",{search:a}).fail(function(){g.query.reset(a)})}),populate:function(a,c){g.tick++;var d=++g.pid;g.currentQuery&&g.currentQuery.abort(),a=i.sanitize(a),b.suggestions().addClass("is-busy"),g.currentQuery=g.query(a).done(function(a){g.pid===d&&(g.set(a),g.suggest(c))
}).fail(function(){g.pid!==d}).always(function(){b.suggestions().removeClass("is-busy"),g.currentQuery=null})},populateTimer:null,populateFromTextfield:function(){g.tick++,clearTimeout(g.populateTimer),setTimeout(function(){var a=b.textfield(),c=i.sanitize(a.val());""===c?(g.hide(),g.populate("",!0)):g.query.cache.hasOwnProperty(c)?g.populate(c):g.populateTimer=setTimeout(function(){g.populate(c)},250)},1)},suggest:function(c){var d=g.tags;if(!(d.length<1)){for(var k,l,n,f=0,j=e.pagination,m=[];j>f&&(k=d[g.i]);)l=k.title,n=h.get(l)||h.create(l),m.push(n),i.get(l)?a(n).addClass("is-used"):f++,g.i++;
b.itemgroup().append(m),c?g.refresh():g.show()}},navigate:function(c){var d=b.item(),e=b.itemgroup(),f=h.activeItem,i=a(f[0]||b.textfield()[0]).nearest(d,c);i.length>0?(h.activate(i),e.scrollIntoView(i)):f.length>0&&("down"==c&&(g.suggest(),e.scrollIntoView(f)),"up"==c&&h.deactivate())},count:function(){var c=a.pluck(g.tags,"title"),d=a.keys(i.itemsByTitle),e=a.without.apply(null,[c].concat(d)),f=e.length;b.total().text(f+"")}},selection:{items:{},activeItem:a(),create:function(a){var b=document.createElement("div");
return b.innerHTML=a,h.items[a]=b},get:function(a){return h.items[a]},activate:function(a){h.deactivate(),h.activeItem=a.addClass("active")},deactivate:function(){h.activeItem.removeClass("active"),h.activeItem=a()},use:function(c){var c=a(c),d=c.text(),e=i.addItem(d);e&&(b.textfield().focus(),c.is(h.activeItem)&&(nextItem=c.next(b.item)[0]||c.prev(b.item)[0],h.deactivate(c),nextItem&&h.activate(a(nextItem))),g.refresh())}},"{self} composerSave":function(c,d,e){if("0"!=EasyBlog.Composer.options.tags.enabled){var f=b.tagger.getAddedItems();
e.data.tags=a.pluck(f,"title").join(",")}}}}),b.resolve()})}),EasyBlog.module("composer/templates",function(a){var b=this;EasyBlog.Controller("Composer.Templates",{defaultOptions:{"{updateTemplate}":"[data-eb-composer-update-template-button]","{templateId}":"[data-eb-composer-template-id]","{saveNewTemplate}":"[data-eb-composer-save-template-button]","{selectTemplate}":"[data-template-item]","{templateBlocks}":"[data-template-blocks]","{deleteTemplate}":"[data-template-delete]","{posts}":"[data-eb-composer-posts]","{searchTextfield}":"[data-eb-composer-posts-search-textfield]","{itemGroup}":"[data-eb-composer-posts-item-group]","{list}":"[data-eb-composer-templates-list]"}},function(b,c,d,e){return{init:function(){e=b.composer
