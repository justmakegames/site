		...
		else
		{
			...
			if (self::$load_com_socialads_assets)
			{
				if (self::$com_socialads_installed == 1)
				{
					...
					$socialadshelper = self::TjloadClass($path, 'socialadshelper');

					if (method_exists($socialadshelper, "getSocialadsJsFiles"))
					{
						// Add component specific js file in provided array && Get first things scripts eg variable declaration.
						$socialadshelper->getSocialadsJsFiles($jsFilesArray, self::$firstThingsScriptDeclaration);
					}
				}
			}
