	...
	<form name="qtcAddProdForm" id="qtcAddProdForm" class="form-validate "
		...
		else
		{
			...
			<div class="">
				...
				<button type="button" class="btn btn-success" title="<?php echo JText::_('QTC_PROD_SVCLOSE')?>" onclick="submitTask('saveAndClose')">
					<?php echo JText::_('QTC_PROD_SVCLOSE')?>
				</button>

				<button type="button" class="btn btn-danger" title="<?php echo JText::_('QTC_COUPON_CANCEL')?>" onclick="submitTask('cancel')">
					<?php echo JText::_('QTC_COUPON_CANCEL')?>
				</button>

				<!--
				<button type="button" class="btn btn-success" title="<?php echo JText::_('QTC_PROD_SVCNEW')?>" onclick="submitTask('saveAndNew')">
					<?php echo JText::_('QTC_PROD_SVCNEW')?>
				</button>
				-->

				<?php
				// IF ITEM SAVED THEN SHOW PREVIEW  //window.parent.document.location.reload(true);
				if (!empty($this->item_id))
				{
					$preveiwLink = JUri::root().'index.php?option=com_quick2cart&view=productpage&layout=default&preview=1&item_id=' . $this->item_id . '&tmpl=component';
					?>
					<a rel="{handler: 'iframe', size: {x: 800, y: 550}, onClose: function(){}}" class="btn btn-primary modal" href="<?php echo $preveiwLink;?>">
						<?php echo JText::_('QTC_PROD_PREVIEW')?>
					</a>
				<?php
				}
				?>
