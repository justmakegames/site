...
<script type="text/javascript">
	...
	*/
	...
	function qtc_generateState(field_name, field_id)
	{
		...
		var country_value=techjoomla.jQuery('#'+countryId).val();
			...
			techjoomla.jQuery.ajax({
						type : "POST",
						url : "<?php echo JUri::base();?>index.php?option=com_quick2cart&task=zoneform.getStateSelectList",
						data : data,
		...
		techjoomla.jQuery.ajax({
			url:root_url+ '?option=com_quick2cart&controller=cartcheckout&task=loadState&country='+country+'&tmpl=component&format=raw',
			type: 'GET',
			dataType: 'json',
	...
	function qtcUpdateZoneRule()
	{
		var data = {
			...
			techjoomla.jQuery.ajax({
				type : "POST",
				url :"<?php echo JUri::base();?>index.php?option=com_quick2cart&task=zoneform.updateZoneRule",
				data : data,
				dataType: "json",
				success : function(response)
				{
	...
	*/
	function qtcDeleteZoneRule(ruleId,delBtn)
	{
			...
			techjoomla.jQuery.ajax({
				type : "POST",
				url : "<?php echo JUri::base();?>index.php?option=com_quick2cart&task=zoneform.deleteZoneRule",
				data : data,
				success : function(response)
				{
					if (response.error!=1)
