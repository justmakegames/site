...
//$this->getTable('shipprofile');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_quick2cart.shipprofile', 'shipprofileform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
...
/*		$db = JFactory::getDBO();
	...
	public function getShipPluginListSelect($default_val='')
	{
		...
		$plugin_list = JHtml::_('select.genericlist', $plugin_options, 'qtcShipPlugin', 'aria-invalid="false" size="1"   autocomplete="off" data-chosen="qtc" onchange=\'qtcLoadPlgMethods()\'', 'value', 'text', $default_val);
		return $plugin_list;
	}
	...
	public function addShippingPlgMeth($update = 0)
	{
		...
		$query->where('sm.client='.$db->Quote($db->escape($data['client'])));
		 $query->where('sm.methodId='.$db->Quote($db->escape($data['methodId'])));		$db->setQuery($query);
