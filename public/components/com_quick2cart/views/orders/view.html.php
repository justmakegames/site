...
class Quick2cartViewOrders extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$this->layout = $layout = $input->get('layout', 'default');
		...
		{
			$orderid       = $input->get('orderid', 0, 'integer');
			$this->orderid = $orderid;
			$input->set('orderid', $orderid);

			if ($layout == 'customerdetails')
			{
				$orderid               = $input->get('orderid', '', 'GET');
			...
			else
			{
				...
				// Check current and logged in user same
				@$this->order_authorized = $this->comquick2cartHelper->getorderAuthorization($order["order_info"][0]->user_id); // out put will be o/1
				...
				{
					$this->storeReleatedView    = 1;
					$allowToViewOrderDetailView = $this->storeHelper->allowToViewStoreOrderDetailView($store_id, $order["order_info"][0]->user_id);
