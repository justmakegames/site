...
-->
	...
	<?php
	if (!empty($vendor_order_view) && empty($orders_email))
	{
	?>
	<div class="row">
		<?php
	...
	if ($orders_email && $this->orderinfo->status == 'P' && !$user->id && $params->get('guest'))
	{
		...
		<div>
			<a href="<?php echo JUri::base() . substr(JRoute::_('index.php?option=com_quick2cart&view=orders&layout=order&email=' . $guest_email . '&orderid=' . $this->orderinfo->id . '&Itemid=' . $Itemid), strlen(JUri::base(true)) + 1);?>">
