...
class Quick2cartControllercartcheckout extends Quick2cartController
{
	...
	public function setCookieCur()
	{
		...
		else
		{
			...
			// Get Item ID
			$iId              = $comquick2cartHelper->getitemid('index.php?option=com_quick2cart&view=cartcheckout');
	...
	public function clearcop()
	{
		...
		if (!empty($cops))
		{
			$copss = array_values($cops);
			$session->set('coupon', $copss);
		}
		else
		{
