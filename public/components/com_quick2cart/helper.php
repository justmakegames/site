...
class Comquick2cartHelper
{
	...
	/*
	 *      [item_name] => Pizza Base - L'exclusif
            [prod_cat] => 15
            [store_id] => 1
            [sku] => Pita Bread - L'Exclusif - 150 gm
            [state] => 1
            [multi_cur] => Array
                (
                    [USD] => 1
            ...
            [description] => Array
                (
                    [data] => Pita Bread - L'Exclusif
                )

            [youtube_link] => https://www.youtube.com/watch?v=zT277Vt69lM
            [stock] => 988
            [min_item] => 1
            [max_item] => 888
            ...
            [781a14111251c5f54419fca533721b65] => 1
            [prodMedia] => Array
                (
                    [0] => Array(.....)
                    ...
                    [1] => Array
                        (
                            ...
                            [file_id] =>
                            [status] => on
                            [mediaFilePath] => 0_dfd9dfff_cb6d_4e4e_8c61_92bab70af669_160x160_0_76247100_1427983182.jpg
                            [purchaseReq] => on
                            [downCount] => -1
                        )
...
*/
	...
	$query="SELECT ug.user_id FROM #__user_usergroup_map as ug LEFT JOIN #__users as u ON ug.user_id=u.id WHERE group_id IN('7','8') AND u.block='0' ";
	 To avoid blocked users
	$db->setQuery($query);
	...
	*/
	...
	public function store_authorize($view_layout, $store_id = "")
	{
		...
		$path    = JPATH_SITE . DS . "components" . DS . "com_quick2cart" . DS . "authorizeviews.php";
		include  $path;
			...
			if (!$error_flag)
			{
				...
				foreach ($img_dimensions_config as $config)
				{
					...
					$upload_image = $media->uploadImage(
									$file_field, $max_zone_width, $max_zone_height, $fullPath, $relPath,
									$colorR, $colorG, $colorB, $new_media_width, $new_media_height,
									$blank_height, $top_offset, $media_extension, $file_name_without_extension
									);
				}
	...
	public function uploadImage($file_field, $oldImgPath = "")
	{
		...
		if (!empty($oldImgPath))
		{
			$oldImgPath = JPATH_SITE . DS . $oldImgPath;
...
//~
	...
	public static function defineIcons($seachLoc = 'SITE')
	{
		...
		{
			require_once JPATH_ADMINISTRATOR . '/components/com_quick2cart/defines.php';
		}
	}
