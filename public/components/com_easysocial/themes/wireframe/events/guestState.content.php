...
<?php } else { ?>
    <?php if (!$event->isOver()) { ?>
        <nav class="media-meta pull-right">
            ...
            <?php if ($this->my->getAccess()->exceeded('events.join', $this->my->getTotalEvents())) { ?>
                <?php echo JText::_('COM_EASYSOCIAL_EVENTS_EXCEEDED_JOIN_LIMIT'); ?>
            <?php } ?>

            <?php } ?>

            <?php if (!$this->my->getAccess()->exceeded('events.join', $this->my->getTotalEvents()) && $event->seatsLeft() !== 0) { ?>
            <?php if ($event->isOpen()) { ?>
