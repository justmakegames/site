...
*/
	...
	function _buildQuery()
	{
		...
		$me=JFactory::getuser();
		$user_id = $me->id;
		$where=" where user_id=".$user_id;//$this->_buildContentWhere();
		$where .=" AND a.status=1";
		if ($layout=='mypayouts')//payouts report //when called from front end
		{
	...
	function getPayoutFormData()
	{
		...
		foreach ($payouts as $index=>$key)
		{
			// calculate toatl total sales
				...
				WHERE o.status='C' AND s.owner =".$key->user_id; // HERE 	USER_ID= STORE OWNER USERID
			...
			if (!empty($userPriceDetail['total_amount']))
			{
				$query="SELECT SUM(amount)
						FROM #__kart_payouts
						WHERE user_id = ".$key->user_id."  AND status = 1";   //owner as user_id
				$this->_db->setQuery($query);
	 ...
	 *	@param  object $post array  also called while saving payout manually and on adaptive payment response
	 *	@return  boolean true/false
	 * */
	function savePayout($post="")
	{
		if (empty($post)) {
	...
	function sendmail($payee_name,$email_id,$transaction_id,$amount,$date,$status,$comment)
	{
		...
		$html = '<br/><div>'.JText::_("CHANGED").'<br/><table><tr><td>'.JText::_("COM_QUICK2CART_TRANSACTION_ID").'</td><td> '.$transaction_id.'</td></tr>
