...
class Quick2cartControllerAttributes extends quick2cartController
{
	...
	function save()
	{
		...
		case 'save':
			...
			if ($result)
			{
				...
				$this->setRedirect($baseURL . "?option=com_quick2cart&view=attributes&layout=attribute&tmpl=component&pid=" . $post->get('product_id') . "&edits=" . $edit . "&client=" . $post->get('client') , $msg);
			}
			else
			{
				$msg = JText::_('QTC_ATTRI_SAVE_PROBLEM');
				$this->setRedirect($baseURL . "?option=com_quick2cart&view=attributes&layout=attribute&tmpl=component&pid=" . $post->get('product_id') . "&client=" . $post->get('client') , $msg);
			}
	...
	function addcurrency()
	{
		...
		$filtered_curr = array_filter($multi_cur, 'strlen'); //  remove empty currencies from multi_curr
		...
		if (!empty($attributes))
		{
			...
			foreach($attributes as $attributes)
			{
				if ($attr_id == $attributes->itemattribute_id)
				{
					...
					$html.= '<td><a  rel="{handler: \'iframe\', size: {x : window.innerWidth-450, y : window.innerHeight-250}, onClose: function(){EditAttribute(' . $attributes->itemattribute_id . ',' . $pid . ');}}" class="btn btn-mini btn-primary modal qtc_modal" href="' . $edit_link . '"> <i class="' . $this->qtc_icon_edit . ' icon-white"></i></a>
				 <button type="button" class="btn btn-mini btn-danger "  onclick=\'deleteAttribute("' . $attributes->itemattribute_id . '","' . $pid . '" )\'><i class="icon-trash icon-white"></i></button>
				 </td>
				</tr>';
				}
		...
		if (!empty($attributes))
		{
			...
			for ($i = 0; $i < count($attributes); $i++)
			{
				if ($i > $count)
				{
					...
					$html.= '<td><a  rel="{handler: \'iframe\', size: {x : window.innerWidth-450, y : window.innerHeight-250}, onClose: function(){EditAttribute(' . $attributes[$i]->itemattribute_id . ',' . $pid . ');}}" class="btn btn-mini btn-primary modal qtc_modal" href="' . $edit_link . '"> <i class="' . $this->qtc_icon_edit . ' icon-white"></i></a>
		...
		if (!empty($getMediaDetail))
		{
			for ($i = 0; $i < count($getMediaDetail); $i++)
			{
				if ($i > $count)
				{
					...
					$html.= '<td>
								<a  rel="{handler: \'iframe\', size: {x : window.innerWidth-400, y : window.innerHeight-200}, onClose: function(){EditFile(' . $getMediaDetail[$i]['file_id'] . ',' . $item_id . ');}}" class="btn btn-mini btn-primary modal qtc_modal" href="' . $edit_link . '"> <i class="' . $this->qtc_icon_edit . ' icon-white"></i>
								</a>
