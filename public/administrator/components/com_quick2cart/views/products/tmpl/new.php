...
<script type="text/javascript">
	...
	Joomla.submitbutton = function(task)
	{
		...
		else if (task != 'products.cancel' && document.formvalidator.isValid(document.id('adminForm')))
		{
			Joomla.submitform(task, document.getElementById('adminForm'));
		}
		else
				...
				<div class="tab-content">
					...
					if ($eProdSupport)
					{
						...
						<div class="tab-pane" id="qtcMediatab">
							<?php
							// Check for view override
							$mediaDetail = $comquick2cartHelper->getViewpath('products', 'medialist', "ADMINISTRATOR", "ADMINISTRATOR");
							ob_start();
							include($mediaDetail);
					...
					if ($isTaxationEnabled  || $qtc_shipping_opt_status)
					{
						...
						<div class="tab-pane" id="taxshipTab">
							<?php
							// Check for view override
							$taxshipPath = $comquick2cartHelper->getViewpath('products', 'taxship', "ADMINISTRATOR", "ADMINISTRATOR");
							ob_start();
							include($taxshipPath);
