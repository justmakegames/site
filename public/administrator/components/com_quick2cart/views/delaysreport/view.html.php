...
class quick2cartViewDelaysreport extends JViewLegacy
{
	...
	{		
		...
		if($layout=='default')
		{
			...
			// $this->sortDirection = $mainframe->getUserStateFromRequest($option.'stat_filter_order_Dir','stat_filter_order_Dir','','word');
			//$this->sortColumn = $mainframe->getUserStateFromRequest($option.'stat_filter_order','stat_filter_order','','string');
	...
	{	
		...
		if($layout=="default")
		{
			...
			if(JVERSION>=3.0)
			{
				...
				JHtmlSidebar::addFilter(JText::_('QTC_SELONE'),'search_select',JHtml::_('select.options',$this->sstatus, "value", "text", $this->lists['search_select'],true)	);
				JHtmlSidebar::addFilter(JText::_('QTC_DAYS'),'search_select_delay',JHtml::_('select.options',$this->delay, "value", "text", $this->lists['search_select_delay'],true)	);
			...
			{
				$button = "<a href='#' onclick=\"javascript:document.getElementById('task').value = 'csvexport';document.getElementById('controller').value = 'salesreport';document.adminForm.submit();\" ><span class='icon-32-save' title='Export'></span>".JText::_('COM_QUICK2CART_SALES_CSV_EXPORT')."</a>";
