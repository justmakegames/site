...
function addopt(){
		...
		var num     = techjoomla.jQuery('.clonedInput').length; // how many 'duplicatable' input fields we currently have
		num = num - 1;
		var newNum  = new Number(num + 1);      // the numeric ID of the new input field being added

		// create the new element via clone(), and manipulate it's ID using newNum value

		var newElem = techjoomla.jQuery('#attri_opts' + num).clone().attr('id', 'attri_opts' + newNum);
...
if (opt_id){
	...
	if (1){
		techjoomla.jQuery.ajax({
			url: '".$qtc_base_url."/index.php?option=com_quick2cart&controller=attributes&task=delattributeoption&opt_id='+opt_id,
			type: 'GET',
...
<form method='POST' name='adminForm' class="form-validate" action='index.php'>
	...
	<div class="row-fluid">
		...
		<div class="span12">
			<div class="span4">
					<strong><?php echo JText::_('QTC_ADDATTRI_NAME'); ?></strong>
			</div>
			<div class="span4">
	...
	<!---s ATTRIBURE NAME -->
			...
			<div class="span4" style="">
				<?php
					...
					echo JHtml::_('select.genericlist', $fields, "att_detail[fieldType]", 'class="no_chzn span6 qtcfieldType" onChange="qtc_fieldTypeChange('.$fnparam.')"', "value", "text",$default);
				?>
			</div>
			<!--- IS COMPULSORY  -->
			<div class="span4 ">
...
?>
	...
	<div class="form-actions">
		...
		<input class="btn btn-default validate" type="button" onclick="qtc_closePopUp();" value="<?php echo JText::_('QTC_ADDATTRI_CANCEL')?>" >		<?php echo JHtml::_( 'form.token' ); ?>
	</div>
