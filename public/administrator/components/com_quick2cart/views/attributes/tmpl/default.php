	...
	function checkForSku(sku)
	{
		...
		if (skuval)
		{
			...
			if (skuval != oldSku)
			{
				...
				var actUrl = "<?php echo $qtc_base_url;?>"+'?option=com_quick2cart&task=attributes.checkSku&sku='+sku;
		...
		<div id='qtcOptionsWraper' style="<?php echo $showQtcFields; ?>">
			<div id='' class=" <?php echo $class_nm;?>" >
				...
				if ($isTaxationEnabled  || $qtc_shipping_opt_status)
				{
					...
					$taxshipPath = $comquick2cartHelper->getViewpath('products', 'taxship', $searchTmpPath, $useViewpath);
					ob_start();
					include($taxshipPath);
