...
class SocialVideo extends EasySocial
{
	...
	// Allowed video types
	private $allowed = array('video/mp4', 'video/ogg', 'video/webm', 'video/x-flv', 'video/3gpp', 'video/quicktime', 'video/x-msvideo', 'video/x-ms-wmv');
	...
	public function snapshot($dimension = '1280x720', $length = '00:00:01')
	{
		...
		if ($this->isLink()) {
			...
			$storage = $storage . '/' . basename($thumbnail);
			...
			$relativePath = $this->getRelativeStoragePath() . '/' . basename($thumbnail);
