...
class JRealtimeControllerGoogle extends JRealtimeController {
	...
	public function display($cachable = false, $urlparams = false) {
		// Access check.
		if (!$this->user->authorise ( 'jrealtime.google', $this->option )) {
	...
	public function deleteEntity() {
		...
		if (! $model->deleteEntity ( null )) {
			...
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_GOOGLE_ERROR_LOGOUT' ) );
			return false;
		}
	
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_GOOGLE_SUCCESS_LOGOUT' ) );
	}
}