<?php
// namespace components\com_jrealtimeanalytics\libraries\framework\controller;
/**
 ...
 * @since 2.0
 */
class JRealtimeController extends JControllerLegacy {
	/**
	 ...
	 * @param Object $params The parameters object with eventual overrides
	 * @return array
	 */
	protected function hasGroupsPermissions($param, $params) {
		$hasPermission = true;
		// Check for live support mode active
		$paramGroups = $params->get($param, array('0'));
		// Live support active!
		if(is_array($paramGroups) && !in_array(0, $paramGroups, false)) {
			// Check for user groups current user belong to
			$userGroups = $this->user->getAuthorisedGroups();
			// Intersect to recognize chat admins
			$intersectResult = array_intersect($userGroups, $paramGroups);
			$hasPermission = (bool)(count($intersectResult));
		}
	...
	public function editEntity() {
		...
		if ($record->checked_out && $record->checked_out != $this->user->id) {
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_CHECKEDOUT_RECORD' ), 'notice' );
			return false;
		}
	...
	public function saveEntity() {
		...
		if (! $result = $model->storeEntity ()) {
			...
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".editEntity&cid[]=" . $this->app->input->get ( 'id' ), JText::_ ( 'COM_JREALTIME_ERROR_SAVING' ) );
			return false;
		}
		...
		$msg = 'COM_JREALTIME_SUCCESS' . $redirects ['msgsufix'];
	...
	public function cancelEntity() {
		...
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_CANCELED_OPERATION' ) );
	}
	...
	public function copyEntity() {
		...
		if (! $model->copyEntity ( $cids )) {
			...
			$modelException = $model->getError ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getErrorLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_ERROR_DUPLICATING' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_SUCCESS_DUPLICATING' ) );
	}
	...
	public function deleteEntity() {
		...
		if (! $model->deleteEntity ( $cids )) {
			...
			$modelException = $model->getError ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getErrorLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_ERROR_DELETE' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_SUCCESS_DELETE' ) );
	}
	...
	public function moveOrder() {
		...
		if (! $model->changeOrder ( $idEntity, $direction )) {
			...
			$modelException = $model->getError ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getErrorLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_ERROR_REORDER' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_SUCCESS_REORDER' ) );
	}
	...
	public function publishEntities() {
		...
		if (! $model->publishEntities ( $idEntity, $this->task )) {
			...
			$modelException = $model->getError ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getErrorLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_ERROR_STATE_CHANGE' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_JREALTIME_SUCCESS_STATE_CHANGE' ) );
	}
