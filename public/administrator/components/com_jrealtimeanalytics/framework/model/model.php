 ...
 * @since 2.0
 */
class JRealtimeModel extends JModelLegacy implements IJRealtimeModel {
	/**
	...
	protected function getExtensionCache() {
		...
		$options = array (
				'defaultgroup' => $this->getState('option', 'com_jrealtimeanalytics'),
				'cachebase' => $conf->get ( 'cache_path', JPATH_CACHE ),
				'lifetime' => (int)$this->componentParams->get('cache_lifetime', 60), // minutes to seconds
		...
		if (isset ( $idEntity ) && $idEntity) {
			try {
				...
				if (! $table->publish( $idEntity, $state )) {
					throw new JRealtimeException ( $table->getError (), 'notice' );
				}
			} catch ( JRealtimeException $e ) {
				$this->setError ( $e );
				return false;
			} catch ( Exception $e ) {
				$jRealtimeException = new JRealtimeException ( $e->getMessage (), 'notice' );
				$this->setError ( $jRealtimeException );
				return false;
			}
		}
			...
			try {
				foreach ( $ids as $id ) {
					if ($table->load ( ( int ) $id )) {
						$table->id = 0;
						$table->name = JText::_ ( 'COM_JREALTIME_COPYOF' ) . $table->name;
						$table->published = 0;
	...
	public function getComponentParams() {
		...
		if($this->app instanceof JApplicationSite && $this->option == 'com_jrealtimeanalytics') {
			$this->componentParams = $this->app->getParams('com_jrealtimeanalytics');
		} else {
			$this->componentParams = JComponentHelper::getParams('com_jrealtimeanalytics');
		}
