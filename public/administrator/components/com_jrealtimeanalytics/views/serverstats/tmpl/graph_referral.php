		...
		<?php foreach ($this->data[REFERRALTRAFFIC] as $referral):?> 
			<tr>
				...
				<?php if(!$this->cparams->get('anonymize_ipaddress', 0)):?>
					<td>
						<?php if($referral[2] == 1):?>
							<a data-title="<?php echo JText::_('COM_JREALTIME_DETAILS');?>" class="hasTooltip" target="_blank" href="http://whois.domaintools.com/<?php echo $referral[1];?>"><?php echo $referral[1];?></a>
						<?php else: ?>
				...
				<?php endif;?>
				<td><?php echo $referral[2];?></td>
				<td><?php printf('%.2f', ($referral[2] / $referral[3]) * 100);?>%</td>
			</tr>
		<?php endforeach;?> 
	</tbody>
</table>