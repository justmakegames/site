...
class JRealtimeModelRealstats extends JRealtimeModel implements IRealstatsModel {
	...
	protected function buildListQueryPagesUserOn() {
		// Build query
		$query = "SELECT COUNT(session_id_person) AS numusers,  DATE_FORMAT(FROM_UNIXTIME(MAX(lastupdate_time)), '%H:%i:%s, %d/%m/%Y') AS lastvisit, nowpage" .
	...
	protected function buildListQueryUsersPageOn() {
		...
		$query = "SELECT stats.nowpage," .
				 "\n stats.current_name," .
				 "\n DATE_FORMAT(FROM_UNIXTIME(stats.lastupdate_time), '%H:%i:%s, %d/%m/%Y') AS lastupdatetime," .
				 "\n sess.username," .
