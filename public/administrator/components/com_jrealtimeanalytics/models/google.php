	...
	private function getSitesProfiles($service, $client, $params) {
		...
		if (count ( $items ) != 0) {
			foreach ( $items as &$profile ) {
				...
				if ($this->purifyDomain ( $profile->getwebsiteUrl () ) == $this->purifyDomain ( $params->get ( 'ga_domain', JUri::root() ) )) {
			...
			<div class="row tablestats no-margin">
				<div class="accordion-group span12">
					...
					<div id="jrealtime_googlestats_graph" class="accordion-body accordion-inner collapse" >
						<div class="btn-toolbar">
							<div class="btn-wrapper"><button class="btn btn-small btn-xs' . ($gaquery == "visitors" ? ' active' : '') . '" onclick="document.getElementById(\'gaquery\').value=\'visitors\'">' . JText::_ ( 'COM_JREALTIME_GOOGLE_METRIC_VISITORS' ) . '</button></div>
							...
							<div class="btn-wrapper"><button class="btn btn-small btn-xs' . ($gaquery == "visitBounceRate" ? ' active' : '') . '" onclick="document.getElementById(\'gaquery\').value=\'visitBounceRate\'">' . JText::_ ( 'COM_JREALTIME_GOOGLE_METRIC_BOUNCERATE' ) . '</button></div>
							...
							<div class="btn-wrapper"><button class="btn btn-small btn-xs' . ($gaquery == "visits" ? ' active' : '') . '" onclick="document.getElementById(\'gaquery\').value=\'visits\'">' . JText::_ ( 'COM_JREALTIME_GOOGLE_METRIC_VISITS' ) . '</button></div>
						</div>
