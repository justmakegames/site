...
<script type="text/javascript">
	...
	function deleteShipProfileMethod(methodId,delBtn)
	{
		...
		techjoomla.jQuery.ajax({
			type : "POST",
			url :"<?php echo JUri::base();?>index.php?option=com_quick2cart&view=shipprofileform&task=shipprofileform.deleteShipProfileMethod",
			data : data,
	...
	function qtcLoadPlgMethods()
	{
		...
		techjoomla.jQuery.ajax({
				type : "POST",
				url :"<?php echo JUri::base();?>index.php?option=com_quick2cart&view=shipprofileform&task=shipprofileform.qtcLoadPlgMethods",
				data : data,
				dataType: "json",
				beforeSend: function()
				{
    ...
    function qtc_addShipMethod()
    {
		...
		techjoomla.jQuery.ajax({
					type : "POST",
					url :"<?php echo JUri::base();?>index.php?option=com_quick2cart&view=shipprofileform&task=shipprofileform.addShipMethod",
					data : data,
					dataType: "json",
					beforeSend: function()
					{
...
<div class=" <?php echo Q2C_WRAPPER_CLASS; ?>">
	...
	<form id="shipprofileform" name="shipprofileform"   method="post" class="form-validate" enctype="multipart/form-data">
				...
				foreach ($this->shipMethods as $meths)
				{
					...
					<tr>
						...
						<td>
							...
							href="index.php?option=com_quick2cart&view=shipprofileform&layout=setrule&id=<?php echo $meths->shipprofile_id;?>&shipmethId=<?php echo $meths->id; ?>&tmpl=component" class="modal">
		...
		<div class="form-horizontal">
			...
			<?php endif; ?>
			 <a href="<?php echo JRoute::_('index.php?option=com_quick2cart&task=shipprofileform.cancel&id='.$this->item->id); ?>&Itemid=<?php echo $store_cp_itemid; ?>" class="btn btn-inverse" title="<?php echo JText::_('COM_QUICK2CART_CANCEL_ITEM'); ?>">
