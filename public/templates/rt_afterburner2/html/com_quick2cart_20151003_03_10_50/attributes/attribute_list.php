...
?>
	<script type="text/javascript">
		...
		function AddNewAttribute(pid,count)
		{
			...
			//alert(rowCount);
			techjoomla.jQuery.ajax({
			url:'<?php echo $qtc_base_url;?>?option=com_quick2cart&task=attributes.AddNewAttribute&pid='+pid+'&count='+rowCount,
			type: 'GET',
			dataType: 'json',
		...
		function deleteAttribute(dellink,pid)
		{
				if (confirm("<?php echo JText::_( 'COM_QUICK2CART_ATTRIBUTE_DELET_CONFIRM' ); ?>") == true)
				{
					...
					techjoomla.jQuery.ajax({
					url:'<?php echo $qtc_base_url;?>?option=com_quick2cart&task=attributes.delattribute&pid='+pid+'&attr_id='+dellink,
					type: 'GET',
					dataType: 'json',
...
{
	?>
	<a rel="{handler: 'iframe', size: {x : window.innerWidth-450, y : window.innerHeight-150}, onClose: function(){AddNewAttribute('<?php echo $item_id; ?>','<?php echo $count_tr;?>');}}" class="btn btn-primary btn-small <?php echo ($button_dis == ""?'modal':$button_dis)?> " href="<?php echo $add_link; ?>">
