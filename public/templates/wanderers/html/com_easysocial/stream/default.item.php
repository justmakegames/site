...
>
	<div class="es-stream" data-stream-item >
		...
		<?php if( FD::user()->id != 0 && ( $this->access->allowed( 'stream.hide' ) || $this->access->allowed( 'reports.submit' ) || $stream->editable || ( $this->access->allowed( 'stream.delete', false ) || FD::user()->isSiteAdmin() ) ) ){ ?>
		...
		<div class="es-stream-meta">
			<div class="media">
				...
				<div class="media-body">
					...
					<?php if ($this->config->get('stream.bookmarks.enabled')) { ?>
					<span class="bookmark pull-left mr-5" data-es-provide="tooltip" data-original-title="<?php echo JText::_('You have bookmarked this stream');?>">
					...
					<div class="es-stream-title">
						...
						<div class="es-stream-meta-footer">
							<time>
								<a href="javascript:void(0)"><?php echo $stream->friendlyDate; ?></a>	
							</time>
						</div>
						<?php } ?>
					</div>
					...
					<div class="es-stream-meta-footer">
						...
						<span class="es-edit-text" data-es-provide="tooltip" data-original-title="<?php echo JText::sprintf('COM_EASYSOCIAL_STREAM_LAST_EDITED_ON', FD::date($stream->edited)->format(JText::_('DATE_FORMAT_LC2')));?>">
