		...
		<?php foreach($data as $category) { ?>
			<div class="profile-item clearfix">
				<div class="profile-head">
					...
					<div class="profile-avatar float-r">
						<i class="pabs"></i>
						<a href="<?php echo EasyBlogRouter::_('index.php?option=com_easyblog&view=categories&layout=listings&id='.$category->id); ?>" class="avatar mtm mrm">
					...
					<div class="profile-info">
						<h3 class="profile-title"><a href="<?php echo EasyBlogRouter::_('index.php?option=com_easyblog&view=categories&layout=listings&id='.$category->id); ?>"><?php echo JText::_( $category->title ); ?></a></h3>
											...
											<li <?php if ($i > $initialLimit) { ?>class="more-activebloggers" style="display: none;"<?php } ?>>
												<div class="pts pbs">
													...
													<?php if ( $system->config->get('layout_avatar') ) { ?>
														<img <?php if ($i <= $initialLimit) { ?> src="<?php echo $member->getAvatar(); ?>" <?php } else { ?> data-src="<?php echo $member->getAvatar(); ?>" <?php } ?> alt="<?php echo $member->getName(); ?>" width="40" height="40" class="avatar" style="overflow: hidden;"/>
