...
EasyBlog.ready(function()
{
	...
	<div class="tag-sort-order clearfix">
		<ul class="reset-ul ">
			...
			<li class="tag-search float-r">
				<input type="text" id="filter-tags" placeholder="Search Tags" name="filter-tags" class="ffa fsg" />
			</li>
	    </ul>
	</div>

	<?php if( $tags ) { ?>
	<ul class="post-tags column3 reset-ul float-li clearfix">
	    <?php foreach($tags as $tag) { ?>
	    <li>
			<a href="<?php echo EasyBlogRouter::_('index.php?option=com_easyblog&view=tags&layout=tag&id=' . $tag->id);?>">
                <?php echo JText::_( $tag->title ); ?><span> (<?php echo $tag->post_count;?>)</span>
            </a>
