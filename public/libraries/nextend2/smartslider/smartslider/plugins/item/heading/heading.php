...
class N2SSPluginItemHeading extends N2SSPluginItemAbstract
{
    ...
    public static function onSmartsliderDefaultSettings(&$settings) {
        ...
        $settings['font'][] = '<param name="item-heading-font" type="font" previewmode="paragraph" label="' . n2_('Item') . ' - ' . n2_('Heading') . '" default="' . self::$font . '" />';
    ...
    function getTemplate($slider) {
        ...
        return "<div><h{priority} id='{{uuid}}' class='{fontclass} {styleclass} {class}' style='display: {display}; {extrastyle};'><a href='#' onclick='return false;'>{heading}</a></h{priority}>" . NHtml::scriptTemplate($this->getJs($slider->elementId, "{{uuid}}")) . "</div>";
    }
    ...
    private function getHtml($data, $id, $slider, $slide, $attributes = array()) {
        ...
        return $this->heading($data->get('priority', 2), $attributes + array(
            ...
            ), $this->getLink($slide, $data, strip_tags($slide->fill($data->get('heading', ''))), $linkAttributes));
    }
