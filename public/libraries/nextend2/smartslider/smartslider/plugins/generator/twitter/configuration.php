...
class N2SliderGeneratorTwitterConfiguration
{
    ...
    public function __construct($info) {
        ...
        $this->configuration->loadJSON(N2Base::getApplication('smartslider')->storage->get('twitter'));
    ...
    public function addData($data, $store = true) {
        ...
        if ($store) {
            N2Base::getApplication('smartslider')->storage->set('twitter', null, json_encode($this->configuration->toArray()));
        }
    }
    ...
    public function finishAuth() {
        ...
        unset($_SESSION['data']);
        try {
            $this->configuration->set('user_token', $_SESSION['t_oauth']['oauth_token']);
            $this->configuration->set('user_secret', $_SESSION['t_oauth']['oauth_token_secret']);
            $client = $this->getApi();
